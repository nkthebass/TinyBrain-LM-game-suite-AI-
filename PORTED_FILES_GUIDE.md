...
## CRITICAL: VRAM Management in the Training Loop

**This is the most common mistake when porting TorchSharp training code. A tiny 9M parameter model can consume 12GB+ of VRAM if this is done wrong.**

TorchSharp does not automatically free GPU tensors when they go out of C# scope. Every intermediate tensor (logits, loss, activations) stays on the GPU until explicitly freed. If you wrap your entire training loop in a single `torch.NewDisposeScope()`, every tensor from every step accumulates in memory for the whole run.

### The Wrong Pattern (causes VRAM explosion):
```csharp
// DON'T DO THIS — holds every tensor from every step until training ends
using var outerScope = torch.NewDisposeScope();

for (int step = 0; step < totalSteps; step++)
{
    var logits = model.forward(input);   // never freed until outerScope ends
    var loss = ComputeLoss(logits, ...); // never freed until outerScope ends
    loss.backward();
    optimizer.step();
}
```

### The Correct Pattern (VRAM stays flat):
```csharp
// Model is created OUTSIDE any dispose scope so it survives training
...
for (int step = 0; step < totalSteps; step++)
{
    // Each step gets its own scope — all intermediate tensors freed at end of step
    using var stepScope = torch.NewDisposeScope();

    optimizer.zero_grad();

    var input  = torch.tensor(inputArr,  new long[] { batch, seqLen }).to(device);
    var target = torch.tensor(targetArr, new long[] { batch, seqLen }).to(device);

    var logits = model.call(input);
    var loss   = model.ComputeLoss(logits, target, lossFn);

    loss.backward();

    torch.nn.utils.clip_grad_norm_(model.parameters().ToList(), 1.0f);
    optimizer.step();

    // stepScope ends here — input, target, logits, loss all freed from GPU
}

// After training, model is still alive (it was never inside a scope)
model.eval();
```

### Key rules:
1. **Model, optimizer, lossFn** — create these **outside** all dispose scopes. They must live for the whole run.
2. **Per-step tensors** (input batches, logits, loss) — always inside a `using var stepScope = torch.NewDisposeScope()` at the top of the loop body.
3. **Never wrap the entire training loop** in a single outer dispose scope.
4. **For inference**, always use `torch.no_grad()` combined with a dispose scope:
   ```csharp
   using var inferScope = torch.NewDisposeScope();
   using (torch.no_grad())
   {
   ...
   ```
5. If VRAM still grows, add `GC.Collect()` every N steps to flush any managed wrappers the .NET GC hasn't collected yet.
...
Draw the board on a custom `Panel` using `OnPaint`:
...
public class BoardPanel : Panel
{
    ...
    // Call board.Invalidate() after updating Board[] to redraw
}
```
...
- "VRAM: model/optimizer created outside dispose scopes, per-step tensors inside `using var stepScope = torch.NewDisposeScope()`"
